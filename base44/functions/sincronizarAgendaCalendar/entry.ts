...
async function sincronizarUnidade(base44, config, unidade_id) {
  ...
  if (!res.ok) {
    ...
    return { sincronizados: 0, ignorados: 0, totalEventos: 0, erro: `Erro ao ler calendário: ${err}` };
  }
  ...
  return { sincronizados, ignorados, totalEventos: events.length };
}
...
export default async function(req) {
  try {
    ...
    // Se unidade_id específica, sincroniza apenas ela
    if (unidade_id) {
      const config = configs.find(c => c.unidade_id === unidade_id) || configs.find(c => !c.unidade_id) || configs[0];
      const result = await sincronizarUnidade(base44, config, unidade_id);
      return Response.json({
        message: `Google Calendar: ${result.sincronizados} novo(s) agendamento(s), ${result.ignorados} ignorado(s).`,
        ...result,
      });
    }

    // Sem unidade_id: sincroniza todas as unidades que têm config
    let totalSincronizados = 0;
    let totalIgnorados = 0;
    let totalEventos = 0;
    for (const config of configs) {
      const uid = config.unidade_id || null;
      const result = await sincronizarUnidade(base44, config, uid);
    ...
    return Response.json({
      message: `Google Calendar: ${totalSincronizados} novo(s) agendamento(s), ${totalIgnorados} ignorado(s).`,
